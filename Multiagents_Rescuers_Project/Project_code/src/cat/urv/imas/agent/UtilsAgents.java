...
public class UtilsAgents {
    ...
    /**
     * To search all agents of a certain type
     *
     * @param parent Agent
     * @param sd ServiceDescription search criterion
     * @return AIDs of the agents if it is foun, it is a *blocking* method
     */
...
/**
     ...
     * @return AID of the agent if it is foun, it is a *blocking* method
     */
    public static Class<?> searchAgentObject(Agent parent, ServiceDescription sd) {
        /**
         * Searching an agent of the specified type
         */
        Class<?> searchedAgent = null;
        DFAgentDescription dfd = new DFAgentDescription();
        dfd.addServices(sd);
        try {
            while (true) {
    ...
    /**
     * IDEA: To create the agent and the container together, returning the container.
     *
     * @param agentName String Agent name
     * @param className String Class
     * @param arguments Object[] Arguments
     * @return AgentContainer created
     */
    public static AgentContainer createAgentsGetContainer(String[] agentNames, String[] classNames, Object[] arguments) {
        AgentContainer container = null;
        int i=0;
        try {
            Runtime rt = Runtime.instance();
            ...
            if (agentNames.length == classNames.length) {
                for (i=0; i<agentNames.length; i++) {
                    AgentController controller = container.createNewAgent(agentNames[i], classNames[i], arguments);
                    controller.start();
                }
            }
        } catch (Exception e) {
            System.err.println("ERROR: Cannot create agent " + agentNames[i] + " of class " + classNames[i]);
            e.printStackTrace();
        }
        return container;
    }
